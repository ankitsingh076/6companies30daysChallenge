class Solution
{
    ...
    int diajstra(int n, int source, int destination, vector<pi> adj[])
    {
        ...
        pq.push({ 0,
            source });	// Push the source node into the queue

        while (!pq.empty())
        {
            auto[cost, node] = pq.top();
            pq.pop();	// Get the node with the smallest distance
            ...
            for (auto[ccost, cnode]: adj[node])
            {
                ...
                if (cost + ccost < distance[cnode])
                {
                    ...
                    pq.push({ distance[cnode],
                        cnode });	// Add the updated distance to the queue
                }
            }
        }
    ...
    vector<vector < int>> modifiedGraphEdges(int n, vector<vector < int>> &edges, int source, int destination, int target)
    {
        ...
        for (auto &e: edges)
        {
            ...
            adj[e[0]].push_back({ e[2],
                e[1] });	// Add edge to the adjacency list
            adj[e[1]].push_back({ e[2],
                e[0] });	// Undirected graph: add both directions
        }
        ...
        for (auto &e: edges)
        {
            ...
            adj[e[0]].push_back({ e[2],
                e[1] });	// Add modified edge to adjacency list
            adj[e[1]].push_back({ e[2],
                e[0] });	// Add reverse edge
